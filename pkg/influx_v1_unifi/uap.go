...
func (u *InfluxV1Unifi) batchRogueAP(r report, s *unifi.RogueAP) {
	if s.Age.Val == 0 {
...
func (u *InfluxV1Unifi) batchUAP(r report, s *unifi.UAP) {
...
func (u *InfluxV1Unifi) processUAPstats(ap *unifi.Ap) map[string]any {
	if ap == nil {
...
func (u *InfluxV1Unifi) processVAPTable(r report, t map[string]string, vt unifi.VapTable) { // nolint: funlen
	for _, s := range vt {
		tags := map[string]string{
			"device_name": t["name"],
...
func (u *InfluxV1Unifi) processRadTable(r report, t map[string]string, rt unifi.RadioTable, rts unifi.RadioTableStats) {
	for _, p := range rt {
		tags := map[string]string{
			"device_name": t["name"],
			"site_name":   t["site_name"],
