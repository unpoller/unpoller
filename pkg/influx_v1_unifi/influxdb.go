// Package influx_v1_unifi provides the methods to turn UniFi measurements into influx
...
// InfluxV1Unifi is returned by New() after you provide a Config.
type InfluxV1Unifi struct {
...
func init() { // nolint: gochecknoinits
	u := &InfluxV1Unifi{InfluxDB: &InfluxDB{}, LastCheck: time.Now()}
...
func (u *InfluxV1Unifi) PollController() {
...
func (u *InfluxV1Unifi) Run(c poller.Collect) error {
	var err error
...
func (u *InfluxV1Unifi) setConfigDefaults() {
	if u.URL == "" {
...
func (u *InfluxV1Unifi) getPassFromFile(filename string) string {
...
func (u *InfluxV1Unifi) ReportMetrics(m *poller.Metrics, e *poller.Events) (*Report, error) {
	r := &Report{
		Metrics: m,
		Events:  e,
		ch:      make(chan *metric),
...
func (u *InfluxV1Unifi) collect(r report, ch chan *metric) {
	for m := range ch {
		if m.TS.IsZero() {
			m.TS = r.metrics().TS
		}
...
func (u *InfluxV1Unifi) loopPoints(r report) {
	m := r.metrics()
...
func (u *InfluxV1Unifi) switchExport(r report, v any) { //nolint:cyclop
	switch v := v.(type) {
	case *unifi.RogueAP:
		u.batchRogueAP(r, v)
	case *unifi.UAP:
